...
actor InterestProfileStore {
    static let shared = InterestProfileStore()
    ...
    func currentProfile() async -> InterestProfile {
        await ensureLoaded()
        return profile
    }

    func applySignal(_ signal: InterestSignal) async {
        await ensureLoaded()
    ...
    func shouldProcessNewsletter(themeTag: String, sender: String) async -> Bool {
        await ensureLoaded()
