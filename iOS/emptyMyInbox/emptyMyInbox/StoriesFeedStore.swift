...
actor StoriesFeedStore {
    static let shared = StoriesFeedStore()
    ...
    func stories() async -> [InsightCard] {
        await ensureLoaded()
        return state.stories
    }

    func promptStates() async -> [Int: StoryPromptState] {
        await ensureLoaded()
        return state.promptStates
    }

    func appendStories(_ newStories: [InsightCard]) async {
        await ensureLoaded()
    ...
    func applyPromptOutcome(_ outcome: StoryPromptOutcome) async {
        await ensureLoaded()
    ...
    func dismissStory(storyId: Int) async {
        await ensureLoaded()
