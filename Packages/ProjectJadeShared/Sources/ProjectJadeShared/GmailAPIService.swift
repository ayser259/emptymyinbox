...
public class GmailAPIService {
    ...
    private let keychainService = "com.projectjade.gmail"
    ...
    private func appSupportAccountsFileURL() -> URL {
        ...
        let dir = base.appendingPathComponent("ProjectJade", isDirectory: true)
    ...
    private func keychainAccountsQuery() -> [String: Any] {
        [
    ...
    /// Upserts Gmail account JSON into the keychain without delete-then-add (avoids `errSecDuplicateItem` on sandboxed macOS).
    private func saveAccountsToKeychain(data: Data) -> OSStatus {
        let query = keychainAccountsQuery()
    ...
    private func saveAccountsWhileLocked() {
        ...
        let status = saveAccountsToKeychain(data: data)
        if status == errSecSuccess {
    ...
    private func loadSavedAccounts() {
        persistenceLock.lock()
        defer { persistenceLock.unlock() }
        var query = keychainAccountsQuery()
    ...
    private func clearSavedAccounts() {
        persistenceLock.lock()
        defer { persistenceLock.unlock() }
        let status = SecItemDelete(keychainAccountsQuery() as CFDictionary)
