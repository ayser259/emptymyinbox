...
public struct ConfigureVaultPanel: View {
    ...
    private func createDriveVault() async {
        #if os(iOS)
        guard let vc = UIApplication.projectJade_topPresentedViewController() else {
...
extension UIApplication {
    ...
    static func projectJade_topPresentedViewController() -> UIViewController? {
        ...
        return root.projectJade_topPresented()
    }
}

private extension UIViewController {
    func projectJade_topPresented() -> UIViewController {
        if let presented = presentedViewController {
            return presented.projectJade_topPresented()
        }
        if let nav = self as? UINavigationController, let visible = nav.visibleViewController {
            return visible.projectJade_topPresented()
        }
        if let tab = self as? UITabBarController, let selected = tab.selectedViewController {
            return selected.projectJade_topPresented()
        }
        return self
    }
}
