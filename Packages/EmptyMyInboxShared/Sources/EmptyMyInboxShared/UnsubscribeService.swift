...
public struct UnsubscribeResult {
    ...
    public let method: UnsubscribeMethod?
    public let details: String?  // Additional details for verification (URL, email address, etc.)
    public let requiresManualAction: Bool  // True if user needs to complete unsubscribe manually
    ...
    public var manualActionURL: URL? {
...
public class UnsubscribeService {
    public static let shared = UnsubscribeService()
    
    private init() {}
    
    /// Extract unsubscribe information from email headers
    /// Returns the unsubscribe method if found, nil otherwise
    public func extractUnsubscribeInfo(from headers: [String: String]) -> UnsubscribeMethod? {
    ...
    public func executeUnsubscribe(method: UnsubscribeMethod, userEmail: String) async -> UnsubscribeResult {
        switch method {
        case .http(let url):
    ...
    public func getUnsubscribeInfo(for email: EmailDetail, accountEmail: String) async -> UnsubscribeMethod? {
        let gmailService = GmailAPIService.shared
    ...
    public func getUnsubscribeInfoForSender(senderEmail: String, accountEmail: String) async -> UnsubscribeMethod? {
        let gmailService = GmailAPIService.shared
        guard let account = gmailService.getAccount(byEmail: accountEmail) else {
            return nil
        }
...
extension GmailAPIService {
    ...
    public func sendEmail(for account: GmailAccount, to: String, subject: String, body: String) async throws {
